...
const DetalhesEstacao = ({ stationId }: DetalhesEstacaoProps) => {
  const { data, isLoading: isFetching, error } = useFetchStations();
  ...
  useEffect(() => {
    if (isFetching) {
      ...
      const foundStation = data.chargingStations.find((s: Station) => s.stationId === stationId);
      setStation(foundStation || null);
      setIsLoading(false);
    }
  }, [data, isFetching, stationId]);
